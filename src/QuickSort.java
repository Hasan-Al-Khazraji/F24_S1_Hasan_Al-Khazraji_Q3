...
public class QuickSort<T extends Comparable<T>> {
    ...
    public static <T extends Comparable<T>> T[] sort(PIVOT_TYPE pivotType, T[] unsorted) {
        ...
        if (pivotType == PIVOT_TYPE.MIDDLE) {
            ...
            pivot = getRandom(unsorted.length); // Out of bounds
        }
    ...
    private static <T extends Comparable<T>> void sort(int index, int start, int finish, T[] unsorted) {
        if (start >= finish)
            return; // Added to handle empty arrays
        ...
        int f = finish;
        while (s <= f) {
            while (unsorted[s].compareTo(pivot) < 0 && s <= finish) // Added fix for s <= finish which was causing ArrayIndexOutOfBoundsException
                s++;
            while (unsorted[f].compareTo(pivot) > 0 && f >= start) // Added fix for f >= start which was causing ArrayIndexOutOfBoundsException
                f--;
            if (s <= f) {
                swap(s, f, unsorted);
                s++;
